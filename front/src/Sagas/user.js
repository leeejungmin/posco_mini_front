...
import { customAxios, customAxiosGraph } from "../Http/customAxios";

// yield all 보고있다가 실행시키겠다
export default function* userSaga() {
  yield all([fork(watchLogin), fork(watchGraph)]);
}
...
function* watchGraph() {
  console.log("saga / watchLogin");
  yield takeLatest(GRAPH_REQUEST, callGraph);
}

function* callGraph(action) {
  try {
    console.log("saga/ call Graph");
    const result = yield call(customAxiosGraph, "get", `/image`, null);
